...
| AI 筛选 | 按日期或全量批量抓正文与图片（长图切片、海报二维码解码），GLM（10 并发）不限专业方向地提取岗位/地点/报名链接，并识别宣讲会/双选会的时间地点；仅纯活动通知与用人单位邀请函跳过；抓取被微信验证页拦截时记为可重试、下次自动补析；分析全员复用，看板的 AI 筛选 chips 可多选叠加（方向关键词/意向城市/含计算机类岗位等，条件取交集） |
| 宣讲会页 | 宣讲会/招聘会/双选会单独一页：按举办日期排时间轴、点哪天加载哪天，省份多选筛选，多企业联合的排省内置顶并加徽标；省份按地点/城市/公众号所属高校推断；不计入文章看板总数 |
...
5. **网页看板**：直接读分组 JSON 渲染，点击行为写 SQLite，不参与抓取与去重逻辑。可选的 AI 筛选（`web/screen_update.py`）在同一界面按日期提取招聘信息与宣讲会详情，结果写 `web/data/screen_results.json` 供看板合并展示。
...
浏览器打开 <http://127.0.0.1:8787>，用刚才的用户名和访问码登录——按日期的招聘清单、已读打勾、宣讲会页、收藏与已投递标记、AI 筛选视图，快照数据上的全部功能都能点。网页服务只用 Python 标准库（Python 3.12+ 即可，无需先装依赖）；用 uv 的话 `uv run python web/server.py serve` 等价。局域网多人访问加 `--host 0.0.0.0`。
